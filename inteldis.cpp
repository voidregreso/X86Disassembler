...
// Tabla de registros de 8 bits
char *g_szRegByte[8] = {
    "AL", "CL", "DL", "BL", "AH", "CH", "DH", "BH"
};

// Tabla de registros de 16 bits
char *g_szRegWord[8] = {
    "AX", "CX", "DX", "BX", "SP", "BP", "SI", "DI"
};

// Tabla de registros de 32 bits
char *g_szRegDword[8] = {
...
DIS_OPCODE_T Group0xff[] = { //inc/dec/Instrucciones de transferencia indirecta/push group
...
DIS_OPCODE_T Group0xd8[] = { //fpu group
    ...
    //Instrucciones de coma flotante de doble byte
...
DIS_OPCODE_T Group0xdf[] = { //fpu group
    // Las instrucciones de coma flotante relacionadas con datos de memoria tienen la misma longitud que las instrucciones (add D__Gb,D__Eb)
...
DIS_OPCODE_T Group0x0f00[] = { // Instrucciones de modo protegido de la CPU
...
DIS_OPCODE_T Group0x0f01[] = { // Instrucciones de modo protegido de la CPU
...
/************************************************************************
Nombre de funci車nㄩBOOL OnDisassembly(INSTRUCTION_INFORMATION *pInstInfo, int &pCodeLen,
                             BYTE *pInbuff, DWORD dwEip, int nMode, DIS_CPU *DisCPU)
Funci車n: Maneja el an芍lisis sint芍ctico de instrucciones
Par芍metro 1: pInstInfo Informaci車n de la instrucci車n - OUT
Par芍metro 2: pCodeLen Longitud de la instrucci車n - OUT
Par芍metro 3: pInbuff Informaci車n sobre la instrucci車n que se va a analizar - IN
Par芍metro 4: dwEip Direcci車n virtual de la instrucci車n actual
Par芍metro 5: nMode Modo actual de desmontaje
Par芍metro 6: Informaci車n de an芍lisis de la instrucci車n DisCPU
Valor de retorno: TRUE para un an芍lisis correcto / FALSE para un an芍lisis fallido
************************************************************************/
BOOL OnDisassembly(INSTRUCTION_INFORMATION *pInstInfo, int &pCodeLen,
                   BYTE *pInbuff, DWORD dwEip, int nMode, PDIS_CPU pTagDisCPU) {
    // Tipo y direccionamiento de la instrucci車n DIS_OPCODE_T
    PDIS_OPCODE_T pIntelOpcode = NULL;
    // DISP_INSTRUCTION_T  Infos de mod reg Sib IMm
...
GET_OPATTR:
    // Determinaci車n de los prefijos y tipos de instrucciones
...
/************************************************************************
Nombre de funci車nㄩint OnGetAddress32Dis(DIS_CPU * pDisCPU, BYTE *pbuff,
                                DISP_INSTRUCTION_T *disp_i)
Funci車n: Obtener el offset del operando
Par芍metro 1: informaci車n de resoluci車n de instrucci車n pDisCPU
Par芍metro 2: datos del operando pbuff
Par芍metro 3: informaci車n sobre el operando disp_i
Valor de retorno: offset del operando de retorno
************************************************************************/
...
/************************************************************************
Nombre de funci車nㄩint OnGetAddress16Dis(DIS_CPU * pDisCPU, BYTE *pbuff,
DISP_INSTRUCTION_T *disp_i)
Funci車n: Obtener el offset del operando
Par芍metro 1: informaci車n de resoluci車n de instrucci車n pDisCPU
Par芍metro 2: datos del operando pbuff
...
int OnGetAddress16Dis(DIS_CPU *pDisCPU, BYTE *pbuff, DISP_INSTRUCTION_T *disp_i) {
    ...
    // Esta direcci車n v芍lida es de 16-bits
    length = disp_i->displacement_size + 1;
    return length;
}

/************************************************************************
Nombre de funci車nㄩint OnProcessOpdata_immde(int nOpdata, BYTE *pInbuff,
                                    DISP_INSTRUCTION_T *disp_i)
Funci車n: Obtener los datos y la longitud del n迆mero inmediato en la instrucci車n
Par芍metro 1: nOpdata datos en la informaci車n opcode
Par芍metro 2: datos del operando pInbuff
Par芍metro 3: informaci車n del opcode disp_i
Valor de retorno: longitud de la parte inmediata de la instrucci車n
************************************************************************/
int OnProcessOpdata_immde(int nOpdata, BYTE *pInbuff, DISP_INSTRUCTION_T *disp_i) {
    ...
    case D__1:
        disp_i->Ib = 1;
        break;
    // N迆mero inmediato despu谷s del opcode
    case D__Ib:
    case D_zIb:
    case D_sIb:
        nOpImmedLen = SIZE_b;
    ...
    case D__Iv:
        ...
        break;
    // Manipulaci車n "Saltar al recuento inmediato"
    case D__Jb:
        nOpImmedLen = SIZE_b;
        disp_i->Ib = GetValue_b(pInbuff);
        break;
    case D__Jv:
        ...
        break;
    // Almacenar comandos sin mod/rm, tipo de palabra o 赤ndice
    case D__Ob:
    case D_rOb:
    case D_wOb:
    case D__Ov:
...
/************************************************************************
Nombre de funci車nㄩbool OnProcessName_dis(INSTRUCTION_INFORMATION* pInstInfo,
                                 DIS_OPCODE_T *pIntelOpcode, int OperandSize,
                                 int AddressSize)
Funci車n: Formatear la informaci車n de las instrucciones
Par芍metro 1: pInstInfo Estructura de la instrucci車n
Par芍metro 2: pIntelOpcode Tipos de piezas OPCODE
Par芍metro 3: OperandSize DISP_INSTRUCTION_T.os32
Par芍metro 4: AddressSize DISP_INSTRUCTION_T.as32
Valor de retorno: true si se ha completado el recorrido/false si las condiciones son insuficientes
************************************************************************/
...
/************************************************************************
Nombre de funci車nㄩint OnProcessOpdata_dis(int nOpdata, POPERAND_ITEM op,
                                  DWORD eip, DISP_INSTRUCTION_T *disp_i)
Funci車n: Formatear la informaci車n de las instrucciones
Par芍metro 1: nOpdata Datos del mensaje opcode
Par芍metro 2: estructura de descripci車n del operando op
Par芍metro 3: direcci車n virtual eip
Par芍metro 4: informaci車n sobre el opcode disp_i
Valor de retorno: longitud del operando
************************************************************************/
...
/************************************************************************
Nombre de funci車nㄩint OnPrintPasm(INSTRUCTION_INFORMATION *pInstInfo, char *pAsm)
Funci車n: Dividir el texto del comando y contar la longitud
Par芍metro 1: estructura de instrucciones pInstInfo
Par芍metro 2: texto de la instrucci車n pAsm
Valor de retorno: longitud del texto de la instrucci車n
************************************************************************/
...
/************************************************************************
Nombre de funci車nㄩvoid OnDispTranDis(DIS_ADDRESS * dis_a, DISP_INSTRUCTION_T *disp_i)
Funci車n: Informaci車n del opcode al operando
Par芍metro 1: informaci車n sobre el operando dis_a
Par芍metro 2: informaci車n sobre el operando disp_i
...
/************************************************************************
Nombre de funci車nㄩint OnPutStr(char *pBuf, char *s)
Funci車n: Copiar texto
Par芍metro 1: pBuf Buf de destino
Par芍metro 2: sSourceBuf
Valor de retorno: length
************************************************************************/
